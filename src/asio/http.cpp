...
auto HttpClient::get(std::string_view url)
    -> Task<Expected<std::tuple<std::unique_ptr<Response>, shared_memory<AsyncReadWriteBase>>>> {
  ...
  builder.set_version(Version::HTTP_1_1);
  ...
  if (uri.scheme == "https") {
    ...
    // CO_TRV(tls_sock, co_await TLSUtils{}.ca2_dyn(tls_ctx_, ais));
    ...
    // CO_ENSURE(ec, "Failed to write request");
    // log_info("Request sent successfully, waiting for response...");
    // auto resp = std::make_unique<Response>();
    // ec = co_await resp->read(*tls_sock);
    // CO_ENSURE(ec, "Failed to read response");
    // co_return std::make_tuple(std::move(resp),
    // std::move(tls_sock).into_dyn<AsyncReadWriteBase>());
    ...
    CO_TRV(socket, co_await AsyncSocketCreatorCompose<TcpIp>().ca2_dyn(ais), "Failed to connect");
    ...
    CO_ENSURE(ec, "Failed to read response");
    co_return std::make_tuple(std::move(resp), std::move(socket).into_dyn<AsyncReadWriteBase>());
  }
  std::unreachable();
}
