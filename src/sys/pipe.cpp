...
async_pipe(std::shared_ptr<sync::Poller>& poller) {
  ...
  if (pipe2(fds, O_NONBLOCK) == -1) {
    LOG2("Failed to create pipe, err: {}", strerror(errno));
    return {io::AsyncDevice<feature::In<std::byte>>(-1, nullptr),
            io::AsyncDevice<feature::Out<std::byte>>(-1, nullptr)};
  }
  ...
  return {io::AsyncDevice<feature::In<std::byte>>(fds[0], read_sem),
          io::AsyncDevice<feature::Out<std::byte>>(fds[1], write_sem)};
}
