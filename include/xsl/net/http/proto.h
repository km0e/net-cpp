...
constexpr std::string_view to_string_view(const Version& version) {
...
static constexpr uint16_t to_index(Status status) {
  switch (status) {
...
constexpr std::string_view to_string_view(const Status& status) {
  return HTTP_STATUS_STRINGS[to_index(status)];
}

constexpr std::string_view to_reason_phrase(Status status) {
...
XSL_NB
template <>
constexpr _net::http::Version from_string_view<_net::http::Version>(std::string_view version) {
...
template <>
constexpr _net::http::Method from_string_view<_net::http::Method>(std::string_view method) {
