...
auto inet4_n2p(const sockaddr_storage &storage, std::string &ip, inet::port_t &port) -> errc {
  ...
  ip.resize_and_overwrite(INET_ADDRSTRLEN, [&](auto data, auto) {
    ...
    if (inet_ntop(AF_INET, &sa->sin_addr, data, INET_ADDRSTRLEN)) {
...
auto inet6_n2p(const sockaddr_storage &storage, std::string &ip, inet::port_t &port) -> errc {
  ...
  ip.resize_and_overwrite(INET6_ADDRSTRLEN, [&](auto data, auto) {
    ...
    if (inet_ntop(AF_INET6, &sa->sin6_addr, data, INET6_ADDRSTRLEN)) {
...
auto inet_n2p(const sockaddr_storage &storage, std::string &ip, inet::port_t &port) -> errc {
  switch (storage.ss_family) {
    case AF_INET:
      return inet4_n2p(storage, ip, port);
    case AF_INET6:
      return inet6_n2p(storage, ip, port);
    default:
  ...
  addr.resize_and_overwrite(INET_ADDRSTRLEN + 6, [&](auto data, auto) {
    ...
    if (inet_ntop(AF_INET, &sa->sin_addr, data, INET_ADDRSTRLEN)) {
      ...
      std::to_chars_result res
          = std::to_chars(data + len + 1, data + INET_ADDRSTRLEN + 6, ntohs(sa->sin_port));
      if (res.ec == std::errc{}) {
...
auto inet6_n2p(const sockaddr_storage &storage, std::string &addr) -> errc {
  ...
  addr.resize_and_overwrite(INET6_ADDRSTRLEN + 8, [&](auto data, auto) {
    auto sa = reinterpret_cast<const sockaddr_in6 *>(&storage);
    if (inet_ntop(AF_INET6, &sa->sin6_addr, data + 1, INET6_ADDRSTRLEN)) {
      data[0] = '[';  // add a leading '['
      auto len = std::strlen(data + 1) + 1;
      data[len] = ']';      // add a trailing ']'
      data[len + 1] = ':';  // add a colon for port
      std::to_chars_result res
          = std::to_chars(data + len + 2, data + INET6_ADDRSTRLEN + 7, ntohs(sa->sin6_port));
      if (res.ec == std::errc{}) {
        return static_cast<std::size_t>(res.ptr - data);
      }
      ec = res.ec;
...
auto inet_n2p(const sockaddr_storage &storage, std::string &addr) -> errc {
  switch (storage.ss_family) {
    case AF_INET:
      return inet4_n2p(storage, addr);
    case AF_INET6:
      return inet6_n2p(storage, addr);
    default:
      return errc::address_family_not_supported;
  }
}
