...
Task<io::Result> recv(RawHandle _raw, std::span<byte> buf, Signal &sig) {
  assert(buf.size() > 0);
  do {
 ...
 */
template <class Signal>
Task<io::Result> imm_recv(RawHandle _raw, std::span<byte> buf, Signal &sig) {
  do {
    ssize_t n = ::recv(_raw, buf.data(), buf.size(), 0);
 ...
 * @return Task<io::Result>
 */
template <class Signal, class SockAddr>
Task<io::Result> recvfrom(RawHandle _raw, std::span<byte> buf, SockAddr &addr, Signal &sig) {
  assert(buf.size() > 0);
 ...
 */
template <class Signal, class SockAddr>
Task<io::Result> imm_recvfrom(RawHandle _raw, std::span<byte> buf, SockAddr &addr, Signal &sig) {
  auto [sockaddr, addrlen] = addr.raw();
  do {
    ssize_t n = ::recvfrom(_raw, buf.data(), buf.size(), 0, &sockaddr, &addrlen);
    if (n >= 0) {
...
struct NetAsyncRx {
  ...
  Task<io::Result> recvfrom(this Self &self, std::span<byte> buf, sys::net::SockAddr<Up> &addr) {
    if constexpr (self.is_connection_based()) {
      return _asio::recvfrom(self.raw(), buf, addr, self.read_signal());
    } else {
 ...
 */
template <class Signal, class SockAddr>
Task<io::Result> sendto(RawHandle _raw, std::span<const byte> data, SockAddr &addr, Signal &sig) {
  auto [sockaddr, addrlen] = addr.raw();
  std::size_t total = 0;
  do {
...
Task<io::Result> send_file(RawHandle _raw, io::WriteFileHint hint, Signal &sig) {
  ...
  co_return io::Result(offset - hint.offset, std::nullopt);
}
...
struct NetAsyncTx {
  ...
  /// @brief Send data to a specific address through a device
  template <class Self, sys::net::SocketTraitsCompatible<typename Self::traits_type> Up>
  Task<io::Result> sendto(this Self &self, std::span<const byte> data,
                          sys::net::SockAddr<Up> &addr) {
    return _asio::sendto(self.raw(), data, addr, self.write_signal());
  }
  /// @brief write file to device
